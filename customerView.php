...
if (!isset($_SESSION['at_user_staff_name'])) {
    ...
    $_SESSION['at_page'] ="customerView.php";
    echo "<script>window.location.assign('#login.php');</script>";
}
$trCust="";
$conn = mysqli_connect($hostDB,$userDB,$passDB,$databaseName);
mysqli_set_charset($conn, "UTF8");
$sql="Select * From b_customer Where active = '1' ";
$result = mysqli_query($conn,$sql);
if($result){
    while($row = mysqli_fetch_array($result)){
        $brName="<a href='#customerAdd.php?custId=".$row["cust_id"]."'>".$row["cust_name_t"]."</a>";
        $trCust .= "<tr><td>".$row["cust_code"]."</td><td>".$brName."</td><td>".$row["cust_address_t"]."</td><td>".$row["tele"]."</td><td>"
                .$row["email"]."</td><td>".$row["tax_id"]."</td><td>".$row["contact_name1"]."</td></tr>";
    }
}
$result->free();
mysqli_close($conn);
...
<section id="widget-grid" class="">
	...
	<div class="row">
		...
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                ...
                                <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                                    <thead>			                
                                            <tr>
                                                    ...
                                                    <th data-class="expand"><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> ชื่อลูกค้า</th>
			...
			<!-- end widget -->
                    <footer>
                        <button type="button" class="btn btn-primary" id="btnCustAdd">
                                    เพิ่มลูกค้า
                        </button>
                    </footer>
		</article>
